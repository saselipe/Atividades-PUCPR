def tela_inicial():
    print("Olá, bem vindo ao sistema de gerenciamento do banco SIS!\n\n")
    print("Você gostaria de acessar qual funcionalidade?\n(1) Solicitar cartão\n(2) Gerenciar usuário\n(3) Configurar chave(PIX)\n")

def painel():
    selecao = 1 # Valor fixo para teste

    if selecao == 1:
        print("Você escolheu a solicitação de cartão!\nEm breve iremos te enviar a data de entrega do mesmo.")
        cartao = 'Mastercard'
        return (f'Seu cartão será bandeira {cartao}. ')
    elif selecao == 2:
        print("Você escolheu o gerenciamento de usuário!\nO que você deseja alterar?\n(1) Nome\n(2) Endereço")
        funcao = 1 # Valor fixo para teste
        if funcao == 1:
            nome = "João da Silva" # Valor fixo para teste
            return (f'O nome atualizado é: {nome}')
        elif funcao == 2:
            endereco = "Rua Principal, 123" # Valor fixo para teste
            return (f'O endereço atualizado é: {endereco}')
    elif selecao == 3:
        funcao = 1 # Valor fixo para teste
        if funcao == 1:
            cpf = "123.456.789-00" # Valor fixo para teste
            return (f'Sua chave (CPF) = {cpf}\n')
        elif funcao == 2:
            email = "exemplo@email.com" # Valor fixo para teste
            return (f'Sua chave (E-mail) = {email}\n')
    else:
        return ("Insira uma opção válida.\n")
painel()